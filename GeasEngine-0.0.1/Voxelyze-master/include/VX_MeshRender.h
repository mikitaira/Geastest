...
public:
	...
	CVX_MeshRender(CVoxelyze* voxelyzeInstance, int indLayer, viewColoring colorScheme = MATERIAL, CVoxelyze::stateInfoType stateType = CVoxelyze::DISPLACEMENT, std::string colorScaleLayer = ""); //!< Initializes this mesh visualization with the specified voxelyze instance. This voxelyze pointer must remain valid for the duration of this object. @param[in] voxelyzeInstance The voxelyze instance to link this mesh object to.
	void generateMesh(viewColoring colorScheme = MATERIAL, CVoxelyze::stateInfoType stateType = CVoxelyze::DISPLACEMENT); //!< Generates (or regenerates) this mesh from the linked voxelyze object. This must be called whenever voxels are added or removed in the simulation.
    ...
    // adding code by jun0
    void  getQuads(std::vector<int>  &array);
    void  getQuadColors(std::vector<float>  &array);
    void  getQuadVoxIndices(std::vector<int>  &array);
    void  getQuadNormals(std::vector<float> &array);
    void  getVertices(std::vector<float> &array);
    void  getLines(std::vector<int> &array);
    void  getVertexLinks(std::vector<CVX_Voxel*>  &array);

public:
	CVoxelyze* vx;
	int layer;
	std::string cScaleLayer;
