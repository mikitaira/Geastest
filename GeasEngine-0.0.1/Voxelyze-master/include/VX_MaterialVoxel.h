...
protected:
	...
	void setOrientation(Quat3D<double> orient) { globalOrient = new Quat3D<double>(orient); }
	Vec3D<float> gravityForce() {return (*globalOrient).RotateVec3D(Vec3D<float>(0,-_mass*9.80665f*gravMult,0));} //!< Returns the current gravitational force on this voxel according to F=ma.
