...
public:
	...
	CVX_Voxel(CVX_MaterialVoxel* material, short indexX, short indexY, short indexZ, short iXoffset, short iYoffset, short iZoffset); //!< Default constuctor. @param [in] material Links this CVX_Material to define the physical properties for this voxel. @param[in] indexX The global X index of this voxel. @param[in] indexY The global Y index of this voxel. @param[in] indexZ The global Z index of this voxel.
	CVX_Voxel(CVX_MaterialVoxel* material, short indexX, short indexY, short indexZ, short indLayer); //!< Default constuctor. @param [in] material Links this CVX_Material to define the physical properties for this voxel. @param[in] indexX The global X index of this voxel. @param[in] indexY The global Y index of this voxel. @param[in] indexZ The global Z index of this voxel.
	...
	short indLayer() { return layer; } //!< Returns the global Z index of this voxel.
	...
	Vec3D<double> originalPosition() const { double s = mat->nominalSize(); return Vec3D<double>(ix*s, iy*s + s / 2, iz*s); } //!< Returns the initial (nominal) position of this voxel.
	...
	void enableFloor(bool enabled) { enabled ? boolStates |= FLOOR_ENABLED : boolStates &= ~FLOOR_ENABLED; } //!< Enables this voxel interacting with the floor at Y=0. @param[in] enabled Enable interaction
	bool isFloorEnabled() const { return boolStates & FLOOR_ENABLED ? true : false; } //!< Returns true of this voxel will interact with the floor at Y=0.
	...
	float floorPenetration() const; //!< Returns the interference (in meters) between the collision envelope of this voxel and the floor at Y=0. Positive numbers correspond to interference. If the voxel is not touching the floor 0 is returned.
	...
	void setGlobalDisplacement(Vec3D<double> displacement) { globalDisplacement = displacement; }
	void setGlobalOrientation(Quat3D<double> orientation) { globalOrientation = orientation; }
	Vec3D<double> getGlobalDisplacement(void) { return globalDisplacement; }
	Quat3D<double> getGlobalOrientation(void) { return globalOrientation; }

private:
	typedef int voxState;
	enum voxFlags { //default of each should be zero for easy clearing
		SURFACE = 1<<1, //on the surface?
		FLOOR_ENABLED = 1<<2, //interact with a floor at y=0?
	...
	Vec3D<double> globalDisplacement;   //current displacement to center position (rigit soft Mix GCS)
	Quat3D<double> globalOrientation;   //current orientation to center position (rigit soft Mix GCS)
	...
	void floorForce(float dt, Vec3D<double>* pTotalForce, float CurPenetration); //modifies pTotalForce to include the object's interaction with a floor. This should be calculated as the last step of sumForce so that pTotalForce is complete.
