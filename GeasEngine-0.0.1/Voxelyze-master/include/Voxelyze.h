...
public:
	...
	void clearAllVoxels(); //! Erases all voxels.
	...
	float currentSimTime() { return currentTime; }
	...
	CVX_Voxel* setVoxel(CVX_Material* material, int xIndex, int yIndex, int zIndex, int layer); //!< Adds a voxel made of material at the specified index. If a voxel already exists here it is replaced. The returned pointer can be safely modified by calling any CVX_Voxel public member function on it. @param[in] material material this voxel is made from. This material must already be a part of the simulation - the pointer will have originated from addMaterial() or material(). @param[in] xIndex the X index of this voxel. @param[in] yIndex the Y index of this voxel. @param[in] zIndex the Z index of this voxel.@param[in] layer the layer index of this voxel.
	...
	CVX_Voxel* voxel(int xIndex, int yIndex, int zIndex, int layer) const { CVX_Voxel* vox = voxels(xIndex, yIndex, zIndex); if (vox != NULL && vox->indLayer() == layer) return vox; else return NULL; } //!< Returns a pointer to the voxel at this location if one exists, or null otherwise. The returned pointer can be safely modified by calling any CVX_Voxel public member function on it. @param[in] xIndex the X index to query. @param[in] yIndex the Y index to query. @param[in] zIndex the Z index to query.
	...
	int maxLayer() const { return maxlayer; } //!< The maximum layer number.
	void setMaxLayer(int layer) { maxlayer = layer; } //!< Set maximum layer number.
	...
	void setGravity(float g=1.0f); //!< Sets the gravitational acceleration to apply to all voxels. Gravity acts in the -Y direction. Set to 0 to disable. @param[in] g Gravitational acceleration in g's. 1 g = -9.80665 m/s^2.
	float gravity(void) const {return grav;} //!< Returns the current gravitational acceleration in g's. 1 g = -9.80665 m/s^2.

	void enableFloor(bool enabled = true); //!< Enables or disables a floor that resists voxel penetration at y=0. @param[in] enabled If true, enables the floor. Otherwise disables it.
	...
	//info
	float stateInfo(stateInfoType info, valueType type, std::string cScaleLayer = ""); //!< Returns a specific piece of information about the current state of the simulation. This method is not gaurenteed threadafe. @param[in] info The class of information desired. @param[in] type The type of the value to be returned.

	void setPosition(Vec3D<double> position) { globalPos = position; }
	void setOrientation(Quat3D<double> orient);
	Vec3D<double> getPosition(void) {return globalPos;}
	Quat3D<double> getOrientation(void) { return *globalOrient; }

	void setOrigin(CVX_Voxel* origin) { _origin = origin; }
	CVX_Voxel* getOrigin(void) { return _origin; }
...
private:
	...
	Vec3D<double> globalPos;
	Quat3D<double> *globalOrient;
	CVX_Voxel* _origin;
	...
	CVX_Voxel* addVoxel(CVX_MaterialVoxel* newVoxelMaterial, int xIndex, int yIndex, int zIndex, int layer); //creates a new voxel if there isn't one here. Otherwise
	...
	void replaceVoxel(CVX_MaterialVoxel* newVoxelMaterial, int xIndex, int yIndex, int zIndex, int layer); //replaces the material of this voxel while retaining its position, velocity, etc.
	...
	CVX_Link* addLink(int xIndex, int yIndex, int zIndex, CVX_Voxel::linkDirection direction, int layer); //adds a link (if one isn't already present) and updates parameters
