...
bool CVoxelyze::readJSON(rapidjson::Value& vxl)
{
	...
	if (vxl.HasMember("gravityAcceleration")) {
		if (!vxl["gravityAcceleration"].IsDouble()) {
			return false;
		}
		else {
			grav = (float)vxl["gravityAcceleration"].GetDouble();
		}
	}

	if (vxl.HasMember("relativeAmbientTemperature")) {
		if (!vxl["relativeAmbientTemperature"].IsDouble()) {
			return false;
		}
		else {
			ambientTemp = (float)vxl["relativeAmbientTemperature"].GetDouble();
		}
	}

	if (vxl.HasMember("floorEnabled")) {
		if (!vxl["floorEnabled"].IsBool()) {
			return false;
		}
		else {
			floor = vxl["floorEnabled"].GetBool();
		}
	}

	if (vxl.HasMember("collisionsEnabled")) {
		if (!vxl["collisionsEnabled"].IsBool()) {
			return false;
		}
		else {
			collisions = vxl["collisionsEnabled"].GetBool();
		}
	}
	...
	if (vxl.HasMember("models") && vxl["models"].IsArray()) {
		rapidjson::Value& models = vxl["models"];


		maxlayer = models.Size();
		...
		for (int j = 0; j < models.Size(); j++) {
			...
			if (models[j].HasMember("voxels") && models[j]["voxels"].IsArray() && models[j]["voxels"].Size() % 4 == 0) {
				rapidjson::Value& v = models[j]["voxels"];

				voxelCount += (int)v.Size() / 4;
		...
		for (int j = 0; j < models.Size(); j++) {
			...
			if (models[j].HasMember("voxels") && models[j]["voxels"].IsArray() && models[j]["voxels"].Size() % 4 == 0) {
				...
				//add 'em!
				//for (int i = 0; i < (int)v.Size() / 4; i++) addVoxel(voxelMats[v[i * 4 + 3].GetInt()], v[4 * i].GetInt() + ixoffset, v[4 * i + 1].GetInt() + iyoffset, v[4 * i + 2].GetInt() + izoffset, ixoffset, iyoffset, izoffset);
				for (int i = 0; i < (int)v.Size() / 4; i++) addVoxel(voxelMats[v[i * 4 + 3].GetInt()], v[4 * i].GetInt(), v[4 * i + 1].GetInt(), v[4 * i + 2].GetInt(), j);
			}
		}
	}
...
#ifdef USE_OMP
#pragma omp parallel for if(linkCount>MINPARALLELLOOP)
#endif
...
#ifdef USE_OMP
#pragma omp parallel for if(voxCount>MINPARALLELLOOP)
#endif

	for (int i = 0; i < voxCount; i++) {
		CVX_Voxel* pV = voxelsList[i];
		pV->setGlobalDisplacement(globalPos);
		pV->setGlobalOrientation(*globalOrient);
		pV->timeStep(dt);
    }
...
void CVoxelyze::clear() //deallocates and returns everything to defaults (except voxel size)
{
	...
	globalOrient = new Quat3D<double>;
}

void CVoxelyze::clearAllVoxels() //deallocates and returns everything to defaults (only voxels)
{
	//delete and remove links
	for (std::vector<CVX_Link*>::iterator it = linksList.begin(); it != linksList.end(); it++) delete *it;
	for (int i = 0; i<3; i++)	links[i].clear();
...
CVX_Material* CVoxelyze::addMaterial(float youngsModulus, float density)
{
	try {
		...
		pMat->setOrientation(*globalOrient);
...
CVX_Voxel* CVoxelyze::setVoxel(CVX_Material* material, int xIndex, int yIndex, int zIndex, int layer)
{
	if (material == NULL) {
		removeVoxel(xIndex, yIndex, zIndex);
		return NULL;
	...
	CVX_Voxel* pV = voxels(xIndex, yIndex, zIndex);
	if (pV != NULL) {
		replaceVoxel((CVX_MaterialVoxel*)material, xIndex, yIndex, zIndex, layer);
		return pV;
	}
	else {
		return addVoxel((CVX_MaterialVoxel*)material, xIndex, yIndex, zIndex, layer);
	}
}
...
CVX_Voxel* CVoxelyze::addVoxel(CVX_MaterialVoxel* newVoxelMaterial, int xIndex, int yIndex, int zIndex) //creates a new voxel if there isn't one here. Otherwise
{
	try {
		...
		pV->pos = Vec3D<double>(xIndex*voxSize, yIndex*voxSize + voxSize / 2, zIndex*voxSize); //set initial voxel location (extrapolate?)
...
CVX_Voxel* CVoxelyze::addVoxel(CVX_MaterialVoxel* newVoxelMaterial, int xIndex, int yIndex, int zIndex, int layer) //creates a new voxel if there isn't one here. Otherwise
{
	try {
		nearbyStale = collisionsStale = true;

		CVX_Voxel* pV = new CVX_Voxel(newVoxelMaterial, xIndex, yIndex, zIndex, layer);
		voxels.addValue(xIndex, yIndex, zIndex, pV); //add to the array
		voxelsList.push_back(pV);
		pV->pos = Vec3D<double>(xIndex*voxSize, yIndex*voxSize + voxSize / 2, zIndex*voxSize); //set initial voxel location (extrapolate?)
		//pV->orient = Quat3D<double>(0.78, 0, 0, 1);
		pV->enableFloor(floor);
		pV->setTemperature(ambientTemp); //add it at environment temperature
		pV->enableCollisions(collisions);
		...
		//add any possible links utilizing this voxel
		for (int i = 0; i<6; i++) { //from X_POS to Z_NEG (0-5 enums)
			addLink(xIndex, yIndex, zIndex, (CVX_Voxel::linkDirection)i, layer);
		}
		return pV;
	}
	catch (std::bad_alloc&) {
...
void CVoxelyze::replaceVoxel(CVX_MaterialVoxel* newVoxelMaterial, int xIndex, int yIndex, int zIndex, int layer)
{
	collisionsStale = true; //new material requires new stiffnesses of contact bonds

							//replace the voxel materrial
	CVX_Voxel* pV = voxel(xIndex, yIndex, zIndex, layer);
	pV->replaceMaterial(newVoxelMaterial);

	//reset all the links involving this voxel
	for (int i = 0; i<6; i++) { //from X_POS to Z_NEG (0-5 enums)
		removeLink(xIndex, yIndex, zIndex, (CVX_Voxel::linkDirection)i);
		addLink(xIndex, yIndex, zIndex, (CVX_Voxel::linkDirection)i, layer); //adds only if a voxel is found
	}
}
...
CVX_Link* CVoxelyze::addLink(int xIndex, int yIndex, int zIndex, CVX_Voxel::linkDirection direction, int layer)
{
	CVX_Link* pL = link(xIndex, yIndex, zIndex, direction);
	if (pL) { return pL; } //if a link already exists... well, then it should be up to date.
	...
	//if (voxel1 == NULL || voxel2 == NULL) return NULL; //if no voxel at either position, don't make a link
	int layer1, layer2;
	if (voxel1 == NULL) return NULL; else layer1 = voxel1->indLayer();
	if (voxel2 == NULL) return NULL; else layer2 = voxel2->indLayer();
	if (layer1 != layer || layer2 != layer) return NULL;
													   //make the link and add it to the array+list
	try {
		CVX_MaterialLink* mat = combinedMaterial(voxel1->material(), voxel2->material());
		pL = new CVX_Link(voxel1, voxel2, mat); //, direction);	//make the new link (change to both materials, etc.
...
void CVoxelyze::setOrientation(Quat3D<double> orient) { 

	globalOrient = new Quat3D<double>(orient); 
	for (std::vector<CVX_MaterialVoxel*>::iterator it = voxelMats.begin(); it != voxelMats.end(); it++) {
		(*it)->setOrientation(*globalOrient);
	}
...
void CVoxelyze::setVoxelSize(double voxelSize) //sets the voxel size.
{
	...
	for (std::vector<CVX_Voxel*>::iterator it=voxelsList.begin(); it != voxelsList.end(); it++){
		CVX_Voxel* pV = (*it);
		//pV->pos *= scaleFactor;
		pV->pos = Vec3D<double>(pV->indexX()*voxSize, pV->indexY()*voxSize + voxSize / 2, pV->indexZ()*voxSize);
...
#ifdef USE_OMP
#pragma omp parallel for  if(colCount>MINPARALLELLOOP)
#endif
...
float CVoxelyze::stateInfo(stateInfoType info, valueType type, std::string cScaleLayer)
{
	float returnVal = 0;
	...
	if (info == STRAIN_ENERGY || info==ENG_STRESS || info==ENG_STRAIN){ //Link properties
		...
		for (std::vector<CVX_Voxel*>::const_iterator it = voxelsList.begin(); it!=voxelsList.end(); it++){ //for each voxel
			if(cScaleLayer != ""){
				std::string sLayer = "," + std::to_string((*it)->indLayer()) + ",";
				if (cScaleLayer.find(sLayer) == std::string::npos) continue;
			}
