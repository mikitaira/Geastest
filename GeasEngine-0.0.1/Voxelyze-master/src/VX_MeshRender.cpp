...
CVX_MeshRender::CVX_MeshRender(CVoxelyze* voxelyzeInstance, int indLayer, viewColoring colorScheme, CVoxelyze::stateInfoType stateType, std::string colorScaleLayer)
{
	vx = voxelyzeInstance;
	layer = indLayer;
	cScaleLayer = "";
	if (colorScaleLayer != "") {
		cScaleLayer = "," + colorScaleLayer + ",";
	}
	generateMesh(colorScheme, stateType);
}

void CVX_MeshRender::generateMesh(viewColoring colorScheme, CVoxelyze::stateInfoType stateType)
{
	vertices.clear();
	vertexLinks.clear();
	quads.clear();
		...
		for (int y=minY; y<minY+sizeY+1; y++){
			for (int x=minX; x<minX+sizeX+1; x++){
				...
				for (int i=0; i<8; i++){ //check all 8 possible voxels that could be connected...
					CVX_Voxel* pV = vx->voxel(x-(i&(1<<2)?1:0), y-(i&(1<<1)?1:0), z-(i&(1<<0)?1:0), layer);
				...
				for (int i=0; i<3; i++){ //look in positive x, y, and z directions
					...
					if (p2Ind != -1){ //for x: voxel(x,y,z) (x,y-1,z) (x,y-1,z-1) (x,y,z-1) -- y: voxel(x,y,z) (x-1,y,z) (x-1,y,z-1) (x,y,z-1) -- z: voxel(x,y,z) (x,y-1,z) (x-1,y-1,z) (x-1,y,z)
						if (vx->voxel(x,			y,			z,         layer) ||
							vx->voxel(x-isY,		y-isX-isZ,	z,         layer) ||
							vx->voxel(x-isY-isZ,	y-isX-isZ,	z-isX-isY, layer) ||
							vx->voxel(x-isZ,		y,			z-isX-isY, layer)) {
	...
	updateMesh(colorScheme, stateType);
}
...
void CVX_MeshRender::updateMesh(viewColoring colorScheme, CVoxelyze::stateInfoType stateType)
{
	...
	for (int i=0; i<vCount; i++){ //for each vertex...
		...
		avgPos /= avgCount;
		// 原位置のオフセット
		CVX_Voxel* oV = vx->getOrigin();
		Vec3D<float> originpPos = Vec3D<float>();
		if (oV != NULL) {
			originpPos = vx->getOrigin()->position();
		}
		vertices[3*i] = avgPos.x - originpPos.x;
		vertices[3*i+1] = avgPos.y - originpPos.y;
		vertices[3*i+2] = avgPos.z - originpPos.z;
	...
	if (colorScheme == STATE_INFO){
		maxVal = vx->stateInfo(stateType, CVoxelyze::MAX, cScaleLayer);
		minVal = vx->stateInfo(stateType, CVoxelyze::MIN, cScaleLayer);
	...
	for (int i=0; i<qCount; i++){
		...
		if (cScaleLayer != "") {
			CVX_Voxel* v = vx->voxel(quadVoxIndices[i]);
			std::string sLayer = "," + std::to_string(v->indLayer()) + ",";
			if (cScaleLayer.find(sLayer) == std::string::npos) {
				r = 1.0f;
				g = 1.0f;
				b = 1.0f;
			}
...
void  CVX_MeshRender::getQuads(std::vector<int>  &array)
{
    int size = quads.size();
    for(int i=0; i<size; i++)
    {
        array.push_back(this->quads[i]);
    }
}

void  CVX_MeshRender::getQuadColors(std::vector<float>  &array)
{
    int size = quadColors.size();
    for(int i=0; i<size; i++)
    {
        array.push_back(this->quadColors[i]);
    }
}

void  CVX_MeshRender::getQuadVoxIndices(std::vector<int>  &array)
{
    int size = quadVoxIndices.size();
    for(int i=0; i<size; i++)
    {
        array.push_back(this->quadVoxIndices[i]);
    }
}

void  CVX_MeshRender::getQuadNormals(std::vector<float> &array)
{
    int size = this->quadNormals.size();
    for(int i=0; i<size; i++)
    {
        array.push_back(this->quadNormals[i]);
    }
}

void  CVX_MeshRender::getVertices(std::vector<float> &array)
{
    int size = this->vertices.size();
    for(int i=0; i<size; i++)
    {
        array.push_back(this->vertices[i]);
    }
}

void  CVX_MeshRender::getLines(std::vector<int> &array)
{
    int size = this->lines.size();
    for(int i=0; i<size; i++)
    {
        array.push_back(this->lines[i]);
    }
}

void  CVX_MeshRender::getVertexLinks(std::vector<CVX_Voxel*>  &array)
{
    int size = vertexLinks.size();
    for(int i=0; i<size; i++)
    {
        array.push_back(this->vertexLinks[i]);
    }
}
